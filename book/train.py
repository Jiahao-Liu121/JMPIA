...
def train(args, data_info, show_loss):
    ...
    for i in range(17860):
        if len(user_interacted_item[i]) > 5:
            indices = np.random.choice(len(user_interacted_item[i]), size=5, replace=False)
            user_interacted_item[i] = [user_interacted_item[i][j] for j in indices]
        elif len(user_interacted_item[i]) > 0:
            indices = np.random.choice(len(user_interacted_item[i]), size=5, replace=True)
            user_interacted_item[i] = [user_interacted_item[i][j] for j in indices]
        else:
            index = item_interact_user_acc_list.index(max(item_interact_user_acc_list))
            t = []
            for j in range(5):
                t.append(index)
            user_interacted_item[i] = t

    for i in range(14967):
        if len(item_interacted_user[i]) > 5:
            indices = np.random.choice(len(item_interacted_user[i]), size=5, replace=False)
            item_interacted_user[i] = [item_interacted_user[i][j] for j in indices]
        elif len(item_interacted_user[i]) > 0:
            indices = np.random.choice(len(item_interacted_user[i]), size=5, replace=True)
            item_interacted_user[i] = [item_interacted_user[i][j] for j in indices]
        else:
            index = user_interact_item_acc_list.index(max(user_interact_item_acc_list))
            t = []
            for j in range(5):
                t.append(index)
            item_interacted_user[i] = t
